...
- (void)playTrack:(Track *)playingTrack;
{
    ...
    playcount = [NSNumber numberWithInt:playcount.intValue+1];

    NSLog(@"Payment count ===============================================>%@",playcount);

    if(playcount.intValue == 4){
        [defaults setObject:@(0) forKey:@"playcount"];
        [defaults synchronize];

        AppDelegate *appDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
        [appDelegate doSomethingAd];

    }
//    else if (playcount.intValue == 1){
//        AppDelegate *appDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
//        [appDelegate prepareAdd];
//        [defaults setObject:@(playcount.intValue) forKey:@"playcount"];
//        [defaults synchronize];
//        
//    }
    else{
